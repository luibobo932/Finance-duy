...
5. **Lệnh tròn số lặp lại (heuristic của chủ danh mục — ưu tiên áp dụng cho CTD)**: trong bảng khớp lệnh, các lệnh khối lượng LỚN + TRÒN SỐ (10.000, 20.000, 50.000, 100.000 cp...) xuất hiện LIÊN TỤC/ĐỀU ĐẶN thường là robot gom hàng của tổ chức/nội bộ — nhà đầu tư nhỏ lẻ đặt lệnh số lẻ, ngẫu nhiên. Khi có dữ liệu khớp lệnh chi tiết, chạy `scripts/tick.py` để đếm tự động; khi chỉ có tin tức, tìm các bài mô tả "lệnh lớn", "khớp chủ động", "phiên có biến" của CTD. Tín hiệu này + giá giữ vững ở vùng hỗ trợ = điểm vào giá tiềm năng (Wyckoff: tích lũy có chủ đích).
