...
| L7 | **`journal.py` đối chiếu khuyến nghị bằng string-matching tiếng Việt cứng** | 🟡 Trung bình | `"CHƯA MUA" in r["rec"].upper()` — dễ vỡ nếu đổi cách viết action. Cần map sang enum hành động chuẩn (HOLD/BUY_SMALL/...). |
| L8 | **Chia hết cho 0 tiềm ẩn** | 🟡 Trung bình | `journal.py` dòng tính `(price - cost) / cost` — nếu `avg_cost == 0` sẽ crash. Chưa xảy ra trong thực tế nhưng cần guard. |
